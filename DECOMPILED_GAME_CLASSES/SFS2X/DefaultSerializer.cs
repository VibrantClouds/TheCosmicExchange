...
// Sfs2X.Protocol.Serialization.DefaultSFSDataSerializer
using System;
...
public class DefaultSFSDataSerializer : ISFSDataSerializer
{
	...
	{
		buffer = typeId switch
		{
			0 => BinEncode_NULL(buffer), 
			1 => BinEncode_BOOL(buffer, (bool)data), 
			2 => BinEncode_BYTE(buffer, (byte)data), 
			3 => BinEncode_SHORT(buffer, (short)data), 
			4 => BinEncode_INT(buffer, (int)data), 
			5 => BinEncode_LONG(buffer, (long)data), 
			6 => BinEncode_FLOAT(buffer, (float)data), 
			7 => BinEncode_DOUBLE(buffer, (double)data), 
			8 => BinEncode_UTF_STRING(buffer, (string)data), 
			20 => BinEncode_TEXT(buffer, (string)data), 
			9 => BinEncode_BOOL_ARRAY(buffer, (bool[])data), 
			10 => BinEncode_BYTE_ARRAY(buffer, (ByteArray)data), 
			11 => BinEncode_SHORT_ARRAY(buffer, (short[])data), 
			12 => BinEncode_INT_ARRAY(buffer, (int[])data), 
			13 => BinEncode_LONG_ARRAY(buffer, (long[])data), 
			14 => BinEncode_FLOAT_ARRAY(buffer, (float[])data), 
			15 => BinEncode_DOUBLE_ARRAY(buffer, (double[])data), 
			16 => BinEncode_UTF_STRING_ARRAY(buffer, (string[])data), 
			17 => AddData(buffer, Array2Binary((ISFSArray)data)), 
			18 => AddData(buffer, Object2Binary((SFSObject)data)), 
			19 => AddData(buffer, Object2Binary(Cs2Sfs(data))), 
	...
	private void ConvertCsObj(object csObj, ISFSObject sfsObj)
	{
		...
		{
			throw new SFSCodecError("Cannot serialize object: " + csObj?.ToString() + ", type: " + fullName + " -- It doesn't implement the SerializableSFSType interface");
		}
		ISFSArray iSFSArray = SFSArray.NewInstance();
		...
		foreach (FieldInfo fieldInfo in array)
		{
			if (!fieldInfo.IsNotSerialized)
			{
				...
				if (sFSDataWrapper == null)
				{
					throw new SFSCodecError("Cannot serialize field of object: " + csObj?.ToString() + ", field: " + name + ", type: " + fieldInfo.GetType().Name + " -- unsupported type!");
				}
	...
	private ISFSArray UnrollArray(ArrayList arr)
	{
		...
		foreach (object item in arr)
		{
			...
			if (sFSDataWrapper == null)
			{
				throw new SFSCodecError("Cannot serialize field of array: " + item?.ToString() + " -- unsupported type!");
			}
			iSFSArray.Add(sFSDataWrapper);
		}
		return iSFSArray;
	...
	private ISFSObject UnrollDictionary(Hashtable dict)
	{
		...
		foreach (string key in dict.Keys)
		{
			...
			if (sFSDataWrapper == null)
			{
				throw new SFSCodecError("Cannot serialize field of dictionary with key: " + key + ", " + dict[key]?.ToString() + " -- unsupported type!");
			}
			iSFSObject.Put(key, sFSDataWrapper);
		}
		return iSFSObject;
	...
	public object Sfs2Cs(ISFSObject sfsObj)
	{
		...
		{
			throw new SFSCodecError("Cannot deserialize object: " + obj?.ToString() + ", type: " + utfString + " -- It doesn't implement the SerializableSFSType interface");
		}
